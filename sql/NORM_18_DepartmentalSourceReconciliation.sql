/* NORM 18 - authoritative departmental comparative and original-budget reconciliation.
   Comparative amounts come from the FY2024 column in the uploaded Defence Annual Report 2023-24.
...
('SOCI','Supplier expenses',N'Supplier expenses',23287040,25057086),
...
('SOCI','Total expenses',N'Total expenses',46336444,49297681),
...
('SOCI','Net cost of services',N'Net cost of services',44012135,47350584),
('SOCI','Revenue from Government',N'Revenue from Government',35840570,38784076),
('SOCI','Operating result',N'(Deficit) / Surplus',-8171565,-8566510),
...
('SOCI','OCI_TOTAL',N'Total comprehensive (loss) / income',-4804011,-8566510),
...
('SOFP','Inventories',N'Inventories',9597327,9664481),
('SOFP','Prepayments',N'Prepayments',3500032,4243307),
('SOFP','TOTAL_NON_FINANCIAL_ASSETS',N'Total non-financial assets',143536774,145340176),
('SOFP','Assets held for sale',N'Assets held for sale',68417,76598),
('SOFP','Total assets',N'Total assets',145895278,148298752),
...
('SOFP','Net assets',N'Net assets',132047045,133350929),
('SOFP','EQUITY_CONTRIBUTED',N'Contributed equity',93451248,107248719),
('SOFP','EQUITY_RETAINED',N'(Accumulated Deficit) / Retained surpluses',1190220,-7935813),
('SOFP','EQUITY_RESERVES',N'Reserves',37405577,34038023),
('SOFP','EQUITY_TOTAL',N'Total equity',132047045,133350929),
('SOFP','Statement of Changes in Equity',N'Total equity',132047045,133350929),
...
('SOCE','SOCE_RETAINED_OPEN',N'Retained earnings - opening balance',9361785,630697),
('SOCE','SOCE_RETAINED_RESULT',N'Retained earnings - deficit',-8171565,-8566510),
('SOCE','SOCE_RETAINED_CLOSE',N'Retained earnings - closing balance',1190220,-7935813),
...
('SOCE','SOCE_TOTAL_OPEN',N'Total equity - opening balance',122550490,128120992),
('SOCE','SOCE_TOTAL_RESULT',N'Total equity - deficit',-8171565,-8566510),
('SOCE','SOCE_TOTAL_OCI',N'Total equity - other comprehensive income',3367554,0),
('SOCE','SOCE_TOTAL_COMPREHENSIVE',N'Total comprehensive (loss) / income',-4804011,-8566510),
('SOCE','SOCE_TOTAL_OWNER',N'Transactions with owners',14300566,13796448),
('SOCE','SOCE_TOTAL_CLOSE',N'Total equity - closing balance',132047045,133350929),
...
DECLARE @AnnualRef NVARCHAR(500)=N'Defence Annual Report 2023-24, departmental primary statements - FY2024 actual column';
...
DECLARE @AnnualUrl NVARCHAR(1000)=N'https://www.defence.gov.au/about/reviews-inquiries/annual-reports';
...
BEGIN
    UPDATE yf SET Amount=f.PriorAmount,LineLabel=f.LineLabel,SourceLocator=N'Controlled uploaded Defence Annual Report 2023-24 FY2024 actual',
    ...
    SELECT @PriorDoc,'PriorActual',f.StatementCode,f.LineCode,f.LineLabel,f.PriorAmount,
        N'Controlled uploaded Defence Annual Report 2023-24 FY2024 actual',100,'Confirmed'
    ...
    UPDATE dbo.tblNORM_YearSetupDocument SET ExtractionStatus='Extracted',
        ...
        ExtractionDetail=N'All departmental comparative figures reconciled to the FY2024 actual column in the uploaded Defence Annual Report 2023-24.'
...
IF (SELECT PriorAmount FROM @Figures WHERE StatementCode='SOCI' AND LineCode='Supplier expenses')<>23287040
    THROW 51046, 'Comparative supplier-expense control failed.', 1;
IF (SELECT PriorAmount FROM @Figures WHERE StatementCode='SOCI' AND LineCode='Operating result')<>-8171565
    THROW 51047, 'Comparative deficit control failed.', 1;
...
IF (SELECT PriorAmount FROM @Figures WHERE StatementCode='SOFP' AND LineCode='Total assets')<>145895278
